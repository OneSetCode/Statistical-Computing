bayesLogistic = function(apredictor,response,data,NumberOfIterations)
{
  ...
  mode = NRMode(apredictor, response, data)
  LaplaceApp = LaplaceApprox(apredictor, response, data, mode) 
  betaEstimate = MHEstimate(apredictor, response, data, mode, NumberOfIterations)
  return(c(LaplaceApp, betaEstimate[1], betaEstimate[2]))
}
...
main('534binarydata-1.txt',10000,10);