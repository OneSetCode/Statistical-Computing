...
//Implement the marglik function using gsl
double marglik(gsl_matrix* data,int lenA,int* A)
{
	...
	int* IndColumnA = new int[lenA];
	for(int i = 0; i < lenA; i++) 
	{
		IndColumnA[i] = A[i] - 1;
	}

	int * IndColumn1 = new int[1];
	IndColumn1[0] = 0;

	D1 = MakeSubmatrix(data, IndRow, n, IndColumn1, 1); 
	DA = MakeSubmatrix(data, IndRow, n, IndColumnA, lenA);

	gsl_matrix* I = gsl_matrix_alloc(lenA, lenA);
	...
	gsl_matrix* tD1_DA = gsl_matrix_alloc(1, lenA);
	matrixproduct(tD1, DA, tD1_DA);

	gsl_matrix* iMA = inverse(MA);

	gsl_matrix* tDA_D1 = gsl_matrix_alloc(lenA, 1);
	matrixproduct(tDA, D1, tDA_D1);

	gsl_matrix* tD1_DA_iMA = gsl_matrix_alloc(1, lenA);
	matrixproduct(tD1_DA, iMA, tD1_DA_iMA);

	gsl_matrix* tD1_DA_iMA_tDA_D1 = gsl_matrix_alloc(1, 1);
	matrixproduct(tD1_DA_iMA, tDA_D1, tD1_DA_iMA_tDA_D1);

	logml = (lgamma((n + lenA + 2.0) / 2.0) - lgamma((lenA + 2.0) / 2.0) - 
	    0.5 * logdet(MA) - ((n + lenA + 2.0) / 2.0) * log(1.0 + gsl_matrix_get(tD1_D1, 0, 0) -
		gsl_matrix_get(tD1_DA_iMA_tDA_D1, 0, 0)));
	...
	gsl_matrix_free(tD1_DA_iMA);
	gsl_matrix_free(tD1_DA_iMA_tDA_D1);
